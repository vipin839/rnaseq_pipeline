...
| Report says `environment.yml — not produced` | The tools were not run from a conda environment (PATH, modules, containers) | Expected; tool versions are in `logs/software_versions.tsv` and `manifest.json` |
| `installed software cannot be used for this step: … is older than the minimum …` | An outdated tool is first on PATH or in the environment | `rnaseq-pipeline --check` shows which; update it from main menu 4. Nothing was run |
| `the project folder is read-only for your user` / `no permission to write …` | The project belongs to another user, or was copied from a read-only disk | Check it without changes with `--validate-project`; to continue, `chmod -R u+w DIR` or copy it somewhere writable |
| `R/DESeq2 step failed: … killed by SIGKILL` | R ran out of memory (most often) | Close other programs or use a machine with more RAM, then re-run DESeq2; partial results are never used |
| `R/DESeq2 step failed: there is no package called …` | The R environment is incomplete | `rnaseq-pipeline --check`, then main menu 4 to repair the R environment |
