...
| 7 | Aligner status shown (validated / not available); only a validated aligner accepted; recorded (plan M4) | `alignment.show_aligners`, `config.validate` | none | MISSING | VERIFIED — `unit::test_only_validated_aligner_accepted`, `integration::test_aligner_status_shown_before_alignment` (fail without the change) |
...
| Project health | per-area summary, scientific validation never PASS if a scientific area fails, blocking / non-blocking issues (plan M7) | MISSING | VERIFIED — `integration::test_project_health_reports_every_area`, `test_project_health_blocking_issue_named` (fail without the change) |
...
| F32 (field finding H14) | LOW (UX) | `cli._project_folder` | ValueError from `validators.existing_dir` not converted | mistyped `--validate-project` path printed a traceback — fixed: clear error listing existing projects; `integration::test_wrong_project_path_is_explained_with_existing_projects` (2 cases, fail without the fix) |
| F30 (P2 finding H12) | HIGH (real data) | `reference_manager.validate_gtf` | strand '?' refused on every record type | the standard NCBI TAIR10.1 annotation could not be used (3 of 806,171 records: trans-spliced organellar transcripts) — fixed: allowed on non-counted types with a note, refused on exon/CDS; `unit::test_unknown_strand_*` (3 tests, fail without the fix) |
| F31 (P2 finding H13) | HIGH (real data) | `reference_manager.stringtie_annotation`, `StringTieStage` | raw provider GTF given to StringTie2, which rejects NCBI gene records (`transcript_id ""`) and strand '?' | StringTie2 failed for all 6 samples — fixed: compatible copy without exactly those records; `integration::test_stringtie_gets_an_annotation_it_can_parse` (real StringTie, verbatim NCBI lines; fails without the fix) |
| F29 (P2 finding H11) | HIGH (operational) | `net.download`, `ena_manager._download_head` | no low-speed timeout on curl; retry budget counted drops that made progress | real-data run stalled with a live-but-silent connection (no data for minutes, curl waiting) — fixed: `--speed-limit 1024 --speed-time 60` + progress-aware retries; `failure::test_stalled_transfer_is_aborted_and_resumed`, `test_drops_with_progress_do_not_exhaust_retries`, `test_no_progress_still_gives_up`; real run resumed the 170.8 MB partial file at ~700 kB/s |
