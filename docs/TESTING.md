...
| `tests/integration` | 71 | the **real interactive CLI** on a synthetic dataset with known truth (16 DE genes): every stage, trimming, strandedness, truth recovery, report/manifest content, no credentials in project, resume without recomputation, dry run, corrupted FASTQ stops the run, an **18-scenario resume-invalidation matrix** (settings changes, corrupted/deleted outputs, edited metadata/results, strandedness and trimming decisions), a changed reference file, a real kill during alignment followed by a resume through the CLI, failure injection (outdated tool, read-only project, low memory, R killed, missing R package), count matrix rebuilt in a moved project, report tampering detection, DESeq2 results re-derived independently (swapped direction, dropped gene, size factor, baseMean, padj), plot data reconciled with the results (volcano, MA, heatmaps, library sizes, correlation), per-sample report numbers re-derived from tool outputs, report check after the project moved, `--validate-project` PASS on a finished project and FAIL after a one-read edit, `--check` mini-job (200/200 pairs counted); plus unstranded / forward / single-end libraries and a deliberately wrong strandedness setting |
...
Counts are those of release 1.1.0 (254 tests; 246 run by default, 8 packaging tests need `RNASEQ_PACKAGING_TESTS=1`).
